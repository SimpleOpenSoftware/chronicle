...
const { useAudioStreamer } = loadTypeScript(sourcePath, {
  ...
  '../../modules/chronicle-duplex-audio': {
    ...
    beginResponse: async () => {}, appendResponse: async () => {}, finishResponse: async () => {},
  },
  '../protocol/audioV2': {
  ...
  '../protocol/nativePlayback': loadTypeScript(path.join(__dirname, '../src/protocol/nativePlayback.ts'), { './audioV2': { AudioCodec: { OPUS: 1 } } }),
