...
class Socket {
  ...
  async connect() {}
  async beginCapture() { this.activeBinding = { captureSessionId: { value: 'capture' }, captureEpoch: 3 }; return this.activeBinding; }
  voiceReady() {}
...
const mocks = {
  ...
  '../../modules/chronicle-duplex-audio': {
    ...
    cancelResponse: async () => {}, beginResponse: async () => {}, appendResponse: async () => {}, finishResponse: async () => {},
  },
...
const nativePath = path.join(__dirname, '../src/protocol/nativePlayback.ts');
...
native.require = request => request === './audioV2' ? protocol : require(request);
native._compile(ts.transpileModule(fs.readFileSync(nativePath, 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020 } }).outputText, nativePath);
mocks['../protocol/nativePlayback'] = native.exports;
